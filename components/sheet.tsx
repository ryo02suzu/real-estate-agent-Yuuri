...
export function Sheet({ title, open, onClose, children }: { title: string; open: boolean; onClose: () => void; children: React.ReactNode }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center" role="dialog" aria-modal="true" aria-label={title}>
      ...
      <div className="relative max-h-[92dvh] w-full max-w-md overflow-y-auto rounded-t-3xl bg-white px-5 pb-[max(20px,env(safe-area-inset-bottom))] pt-4 shadow-xl [@media(max-height:720px)]:pt-2">
        <div className="mx-auto mb-3 h-1 w-10 rounded-full bg-line [@media(max-height:720px)]:mb-1" />
